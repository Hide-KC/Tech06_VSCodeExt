...
@<code>{vsce} で VSIX ファイルをコンパイルするときは、
@<code>{publisher} という発行者の情報が必要になります。
...
拡張機能をオフラインでインストールするときは、 VSIX というファイルが必要になります。 @<fn>{urawaza}
VSIX は拡張機能をコンパイルしたファイルで、後述の @<code>{vsce} モジュールにより生成します。

//footnote[urawaza][どこかで見かけたんですが、~/user/.vscode/extensions にプロジェクトフォルダをコピペしても一応使えるようにはなるそうです……正常に動くのかはわかりません(´・ω・`)]
...
VSIX からのインストールをクリックし、 @<code>{.vsix} ファイルを選択すれば OK です。
